...
string triggerGenerator(assertions asser, string &dec)
{
	...
	dec.insert(dec.find("parameter"), "wire en_n");
	dec.insert(dec.find("parameter"), id);
	dec.insert(dec.find("parameter"), "; //Generated by [ TRIGGER ]\n");

	string triSignal = asser.triSignal;
	if(triSignal.find("in") != string::npos) triSignal.insert(triSignal.find("["), "2");
	...
	trigger.append("    last_");trigger.append(sTriSig);trigger.append(" <= ");trigger.append(triSignal);trigger.append(";\n");
	trigger.append("end\n");
	...
	trigger.append(" = en");
	trigger.append(id);
	if(asser.trigger) {trigger.append(" | ((~last_");trigger.append(sTriSig);trigger.append(" & ");trigger.append(triSignal);trigger.append(") & PPI");trigger.append(id);trigger.append(");\n");}
	else {trigger.append(" | ((last_");trigger.append(sTriSig);trigger.append(" & ~");trigger.append(triSignal);trigger.append(") & PPI");trigger.append(id);trigger.append(");\n");}
	trigger.append("\n");
