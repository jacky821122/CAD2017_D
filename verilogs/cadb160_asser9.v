...
always @(posedge clk) begin
    if(count9>=3'b000) begin //delayFirst: 0
        if(count9<=3'b101) //delaySecond: 5     bit: 3
            co9=1;
        else begin
            co9=0;
        end
...
always @(posedge clk) begin
    last_res_9 <= out[5];
    if(co9 & en_n9) detect9 <= detect_n9;
    else detect9 <= detect9;
    clk_start9 <= clk_start_n9;
end

assign detect_n9 = detect9 | (last_res_9 & ~out[5]);
assign clk_start_n9 = clk_start9 | co9 & en_n9;
assign z9 = (~detect_n9 & (clk_start_n9 & ~co9));
endmodule
