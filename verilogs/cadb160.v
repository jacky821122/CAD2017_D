module fsm(clk, rst, PPI1, PPI2, PPI3, PPI4, PPI5, PPI6, PPI7, PPI8, PPI10, Z, in);
input clk, rst;
input [6:0] in;
reg [6:0] in2;
reg [6:0] out;
...
input PPI1, PPI2, PPI3, PPI4, PPI5, PPI6, PPI7, PPI8, PPI10;
wire z1, z2, z3, z4, z5, z6, z7, z8, z10;
output Z;
assign  Z = z1 & z2 & z3 & z4 & z5 & z6 & z7 & z8 & z10;
...
assign en_n1 = en1 | ((~last_tri_1 & in2[6]) & PPI1);
...
always @(posedge clk) begin
    if(count1>=3'b010) begin //delayFirst: 2
        if(count1<=3'b011) //delaySecond: 3     bit: 3
            co1=1;
        else begin
            co1=0;
        end
...
always @(posedge clk) begin
    last_res_1 <= out[1];
    if(co1 & en_n1) detect1 <= detect_n1;
    else detect1 <= detect1;
    clk_start1 <= clk_start_n1;
end

assign detect_n1 = detect1 | (~last_res_1 & out[1]);
assign clk_start_n1 = clk_start1 | co1 & en_n1;
assign z1 = (~detect_n1 & (clk_start_n1 & ~co1));
...
assign en_n2 = en2 | ((last_tri_2 & ~in2[5]) & PPI2);
...
always @(posedge clk) begin
    if(count2>=4'b0110) begin //delayFirst: 6
        if(count2<=4'b1001) //delaySecond: 9     bit: 4
            co2=1;
        else begin
            co2=0;
        end
...
always @(posedge clk) begin
    last_res_2 <= out[6];
    if(co2 & en_n2) detect2 <= detect_n2;
    else detect2 <= detect2;
    clk_start2 <= clk_start_n2;
end

assign detect_n2 = detect2 | (~last_res_2 & out[6]);
assign clk_start_n2 = clk_start2 | co2 & en_n2;
assign z2 = (~detect_n2 & (clk_start_n2 & ~co2));
...
always @(posedge clk) begin
    if(count4>=6'b011101) begin //delayFirst: 29
        if(count4<=6'b011110) //delaySecond: 30     bit: 6
            co4=1;
        else begin
            co4=0;
        end
...
always @(posedge clk) begin
    last_res_4 <= out[5];
    if(co4 & en_n4) detect4 <= detect_n4;
    else detect4 <= detect4;
    clk_start4 <= clk_start_n4;
end

assign detect_n4 = detect4 | (~last_res_4 & out[5]);
assign clk_start_n4 = clk_start4 | co4 & en_n4;
assign z4 = (~detect_n4 & (clk_start_n4 & ~co4));
...
always @(posedge clk) begin
    if(count5>=3'b000) begin //delayFirst: 0
        if(count5<=3'b010) //delaySecond: 2     bit: 3
            co5=1;
        else begin
            co5=0;
        end
...
always @(posedge clk) begin
    last_res_5 <= out[4];
    if(co5 & en_n5) detect5 <= detect_n5;
    else detect5 <= detect5;
    clk_start5 <= clk_start_n5;
end

assign detect_n5 = detect5 | (~last_res_5 & out[4]);
assign clk_start_n5 = clk_start5 | co5 & en_n5;
assign z5 = (~detect_n5 & (clk_start_n5 & ~co5));
...
assign en_n6 = en6 | ((~last_tri_6 & in2[0]) & PPI6);
...
always @(posedge clk) begin
    if(count6>=5'b10011) begin //delayFirst: 19
        if(count6<=5'b11001) //delaySecond: 25     bit: 5
            co6=1;
        else begin
            co6=0;
        end
...
always @(posedge clk) begin
    last_res_6 <= out[2];
    if(co6 & en_n6) detect6 <= detect_n6;
    else detect6 <= detect6;
    clk_start6 <= clk_start_n6;
end

assign detect_n6 = detect6 | (~last_res_6 & out[2]);
assign clk_start_n6 = clk_start6 | co6 & en_n6;
assign z6 = (~detect_n6 & (clk_start_n6 & ~co6));
...
always @(posedge clk) begin
    if(count7>=4'b1010) begin //delayFirst: 10
        if(count7<=4'b1010) //delaySecond: 10     bit: 4
            co7=1;
        else begin
            co7=0;
        end
...
always @(posedge clk) begin
    last_res_7 <= out[1];
    if(co7 & en_n7) detect7 <= detect_n7;
    else detect7 <= detect7;
    clk_start7 <= clk_start_n7;
end

assign detect_n7 = detect7 | (~last_res_7 & out[1]);
assign clk_start_n7 = clk_start7 | co7 & en_n7;
assign z7 = (~detect_n7 & (clk_start_n7 & ~co7));
...
always @(posedge clk) begin
    if(count8>=3'b100) begin //delayFirst: 4
        if(count8<=3'b101) //delaySecond: 5     bit: 3
            co8=1;
        else begin
            co8=0;
        end
...
always @(posedge clk) begin
    last_res_8 <= out[6];
    if(co8 & en_n8) detect8 <= detect_n8;
    else detect8 <= detect8;
    clk_start8 <= clk_start_n8;
end

assign detect_n8 = detect8 | (last_res_8 & ~out[6]);
assign clk_start_n8 = clk_start8 | co8 & en_n8;
assign z8 = (~detect_n8 & (clk_start_n8 & ~co8));
...
always @(posedge clk) begin
    if(count10>=4'b0010) begin //delayFirst: 2
        if(count10<=4'b1001) //delaySecond: 9     bit: 4
            co10=1;
        else begin
            co10=0;
        end
...
always @(posedge clk) begin
    last_res_10 <= out[2];
    if(co10 & en_n10) detect10 <= detect_n10;
    else detect10 <= detect10;
    clk_start10 <= clk_start_n10;
end

assign detect_n10 = detect10 | (~last_res_10 & out[2]);
assign clk_start_n10 = clk_start10 | co10 & en_n10;
assign z10 = (~detect_n10 & (clk_start_n10 & ~co10));
